			...
			<div>
...

				{
					this.state.is_submit ?
						...
						<List>
							...
							<Item>
								<span className="index-title">老师: </span>{this.state.course.course_teacher}
							</Item>
							<Item>
							...
							{
								this.state.course.course_image_file == null ?
									''
									:
									<Item>
										<img style={{width: '100%', height: '100%'}}
											 src={'http://api.chuangshi.nowui.com' + this.state.course.course_image_file.file_original_path}/>
									</Item>
							}
						</List>
...

				{
					this.state.is_submit && this.state.course_apply_history_status === 'WAITING' ?
						<Result
							img={<img src={require('../assets/svg/waiting.svg')}
									  style={{width: '1.2rem', height: '1.2rem'}} alt=""/>}
							title="平台处理中"
...

				{
					this.state.is_submit && this.state.course_apply_history_status === 'FAIL' ?
						<Result
							img={<Icon type="cross-circle-o" className="icon"
									   style={{fill: '#F13642', width: '1.2rem', height: '1.2rem'}}/>}
							title="申请失败"
